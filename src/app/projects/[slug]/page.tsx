...
export default async function ProjectDetailPage({ params }: Props) {
  ...
  return (
    <div className="bg-cream min-h-screen font-sans">
      ...
      <section className="px-8 pt-9 pb-13 border-b border-cream-border grid grid-cols-[1.2fr_.8fr] gap-13 items-start">
        <div>
          <div className="flex gap-2 mb-4">
            {project.status === 'ongoing' && (
              <span className="inline-flex items-center gap-1.75 text-[11px] tracking-[.08em] uppercase bg-[#FEF3E2] text-[#854F0B] px-3 py-1">
          ...
          <h1 className="font-serif text-[40px] font-bold leading-[1.15] text-ink mb-3.5">
            {project.name}
          </h1>
          <p className="text-[15px] text-ink-muted leading-[1.7] mb-7 max-w-95">
            {project.tagline}
          </p>

          <div className="flex flex-wrap gap-1.75 mb-7">
            {project.tech.map(t => (
              <span
                key={t}
                className="text-[11px] px-2.75 py-1 bg-cream-muted text-ink-secondary border border-cream-border"
              >
                {t}
              </span>
            ))}
          ...
          <div className="flex gap-4">
            {project.links.live && (
              <a
                ...
                className="bg-ink text-cream px-5.5 py-2.5 text-[13px] tracking-[.03em]"
              >
                Live Demo
              </a>
            )}
            {project.links.github && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="border border-ink text-ink px-5.5 py-2.5 text-[13px] tracking-[.03em]"
              >
                GitHub →
              </a>
            )}
      ...
      <section className="px-8 py-13 grid grid-cols-[1.4fr_.6fr] gap-13 items-start border-b border-cream-border">
        <div>
          <div className="bg-cream-surface border border-cream-border p-6 mb-10">
            <p className="text-[11px] tracking-widest uppercase text-ink-muted mb-3">
              The Problem
            </p>
          ...
          <div className="grid grid-cols-2 gap-3 mb-10">
            {stats.map(({ label, value }) => (
              <div key={label} className="bg-cream-surface border border-cream-border p-4.5">
                <p className="text-[11px] tracking-[.06em] uppercase text-ink-muted mb-1.5">
                  {label}
                </p>
        ...
        <div>
          <div className="bg-cream-surface border border-cream-border p-5.5 mb-4">
            <p className="text-[11px] tracking-widest uppercase text-ink-muted mb-3.5">
              What I learned
            </p>
            <ul className="list-none">
              {project.learnings.map((item, i) => (
                <li
                  key={i}
                  className="text-[13px] text-ink-secondary leading-[1.6] py-1.75 border-b border-cream-muted flex items-start gap-2 last:border-b-0 last:pb-0"
                >
                  <span className="text-cream-placeholder shrink-0 mt-px">—</span>
                  {item}
                </li>
              ))}
            </ul>
          ...
          <div className="bg-cream-surface border border-cream-border p-5.5">
            <p className="text-[11px] tracking-widest uppercase text-ink-muted mb-3.5">
              Tech Stack
            </p>
            <ul className="list-none">
